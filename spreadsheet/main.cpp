...
void TestCellReferences() {
    ...
    auto cell = sheet->GetCell("A1"_pos);
    auto vec = cell->GetReferencedCells();
...
void PrintSheet1(const std::unique_ptr<SheetInterface>& sheet) {
    std::ofstream out("toster1.txt");
    out << sheet->GetPrintableSize() << std::endl;
    sheet->PrintTexts(out);
    //out << std::endl;
    //sheet->PrintValues(out);
