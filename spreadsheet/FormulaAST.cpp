...
public:
    ...
    virtual double Evaluate(/*добавьте сюда нужные аргументы*/ std::function<const CellInterface*(Position)> functor) const = 0;
...
public:
    ...
    double Evaluate(/*добавьте нужные аргументы*/std::function<const CellInterface*(Position)> functor) const override {
        // Скопируйте ваше решение из предыдущих уроков.
        double res = 0;
        switch (type_)
        {
        case BinaryOpExpr::Add:
            res = lhs_.get()->Evaluate(functor) + rhs_.get()->Evaluate(functor);
            break;
...
public:
    ...
    double Evaluate(/*добавьте нужные аргументы*/ std::function<const CellInterface*(Position)> functor) const override {
        ...
        case UnaryOpExpr::UnaryPlus:
            res = operand_.get()->Evaluate(functor);
            break;
        case UnaryOpExpr::UnaryMinus:
            res = -1 * operand_.get()->Evaluate(functor);
            break;
        default:
            break;
        }
...
double FormulaAST::Execute(/*добавьте нужные аргументы*/ std::function<const CellInterface*(Position)> functor) const {
    return root_expr_->Evaluate(/*добавьте нужные аргументы*/ functor);
}
